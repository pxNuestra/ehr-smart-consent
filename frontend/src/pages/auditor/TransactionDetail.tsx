  ...
  return (
    <div className="space-y-6 pb-20 lg:pb-6">
      <div>
        ...
        <p className="text-slate-500">Blockchain transaction and audit metadata</p>
      </div>

      {!paramHash && (
        <Card>
          <div className="flex gap-2">
            <Input placeholder="Enter transaction hash..." value={searchHash} onChange={(e) => setSearchHash(e.target.value)} />
            <Button onClick={() => refetch()}>Lookup</Button>
          </div>
        </Card>
      )}
      ...
      {isLoading ? <LoadingSkeleton /> : data && (
        <>
          <Card>
            <CardHeader title="Transaction" subtitle={data.txHash} />
            <dl className="grid gap-3 text-sm md:grid-cols-2">
              <div><dt className="text-slate-500">Hash</dt><dd className="break-all font-mono text-xs">{data.txHash}</dd></div>
              <div><dt className="text-slate-500">Block</dt><dd>{data.receipt?.blockNumber ?? 'N/A'}</dd></div>
              <div><dt className="text-slate-500">Status</dt><dd>{data.receipt?.status === 1 ? 'Success' : 'Pending/Simulated'}</dd></div>
            </dl>
          </Card>
              ...
              <ul className="space-y-2 text-sm">
                {data.auditEntries.map((e: { id: string; action: string; decision: string; metadataHash: string; actor?: { username: string } }) => (
                  <li key={e.id} className="rounded border p-3">
                    <strong>{e.action}</strong> — {e.actor?.username} — {e.decision}
                    <p className="mt-1 font-mono text-xs text-slate-500">{e.metadataHash}</p>
                  </li>
                ))}
              </ul>
            ) : <p className="text-sm text-slate-500">No audit entries for this hash</p>}
          </Card>

          <Card>
            <CardHeader title="Consent Entries" />
              ...
              <ul className="space-y-2 text-sm">
                {data.consentEntries.map((c: { id: string; accessScope: string; purpose: string; status: string; metadataHash: string }) => (
                  <li key={c.id} className="rounded border p-3">
                    Scope: {c.accessScope} | Purpose: {c.purpose} | Status: {c.status}
                    <p className="mt-1 font-mono text-xs text-slate-500">Meta: {c.metadataHash}</p>
                  </li>
                ))}
              </ul>
            ) : <p className="text-sm text-slate-500">No consent entries</p>}
          </Card>
        </>
      )}

      <Link to="/auditor/logs" className="text-primary-600 hover:underline text-sm">← Back to Audit Logs</Link>
    </div>
  );
}
